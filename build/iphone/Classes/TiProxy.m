...
//Rare exceptions to indicate a bug in the _biretreat2015 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_biretreat2015 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_biretreat2015]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.biretreat2015.module"
